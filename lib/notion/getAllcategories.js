...
export async function getAllcategories(posts) {
  ...
  const categoriesConfig = (await getAllPosts({ onlyHidden: false }))
    .filter((p) => p.type && p.type.includes('Category'))
    .sort((a, b) => a.slug - b.slug)
    .map((post) => ({
      title: post.title,
      icon: post.summary,
      count: post.slug === '0' ? sumCount : categoryobj[post.title],
      showAll: post.slug === '0'
    }))
